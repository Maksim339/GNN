...
def solve_transport(
    cell_centers, face_info, k_cells, phi, Sw, p, mu_w, mu_o, wells, dt
):
    M = len(cell_centers)
    net_flux = np.zeros(M)
    eps = 1e-12
    ...
    lambda_w = krw / mu_w
    lambda_o = kro / mu_o
    f_w = lambda_w / (lambda_w + lambda_o + eps)
    ...
    for i in range(M):
        for j, normal, L_edge in face_info[i]:
            ...
            if d < eps:
                continue
            k_avg = harmonic_avg(k_cells[i], k_cells[j], eps)
            lambda_t_i = lambda_w[i] + lambda_o[i]
            lambda_t_j = lambda_w[j] + lambda_o[j]
            lambda_t_avg = harmonic_avg(lambda_t_i, lambda_t_j, eps)
            T_ij = (L_edge / d) * k_avg * lambda_t_avg
            F_total = T_ij * (p[i] - p[j])
            if F_total >= 0:
                f_up = f_w[i]
            else:
                f_up = f_w[j]
...
def plot_results(points, triangles, cell_pressure, cell_saturation):
    ...
    plt.colorbar(tcf, label="Pressure")
...
Sw = np.full(len(cell_centers), 0.001)
Sw[wells["inj"]["cell"]] = 0.7
...
for step in range(num_steps):
    ...
    Sw_new = solve_transport(
        cell_centers, face_info, k_cells, phi_cells, Sw, p, mu_w, mu_o, wells, dt
    )
    Sw = Sw_new
    ...
    mesh_out = meshio.Mesh(
        points,
        [("triangle", triangles)],
        cell_data={"Pressure": [p], "Saturation": [Sw]},
    )
    filename = f"time_step/results_step_{step + 1:04d}.vtk"
